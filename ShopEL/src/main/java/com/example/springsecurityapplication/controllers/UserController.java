...
//    метод для корзины при нажатии добавить в корзину
//    из сылки полчаем айди, потом по айди получаем продукт, извлекаем из сессии объект, у объекта получаем айди, создаем объект корзины, в нее передаем айди пользователя который хочет добавить товар в корзину и айди самого продукта который добавляет, вызываем метод сэйв репозитория и помещаем туда корзину
    @GetMapping("/cart/add/{id}")
    public String addProductInCart(@PathVariable("id") int id, Model model){
        Product product = productService.getProductId(id);
//        извлекаем обект авторизированного пользователя из сессии
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
//
//        // Преобразовываем объект аутентификации в специальный объект класса по работе с пользователями
        PersonDetails personDetails = (PersonDetails) authentication.getPrincipal();
...
//         получаем роль аутентифицированного пользователя
        String role =personDetails.getPerson().getRole();
        int id_person = personDetails.getPerson().getId();
//        получаем корзину пользователя по айди
        List<Cart> cartList = cartRepository.findByPersonId(id_person);
//        создаем лист продуктов потом перебираем корзину добавлем элементы в лист, обращаемся к сирвису вызываем метод по получению продукта по айди (на вход передает айди продукта из корзины) потом кладем в модель и возвращаем шаблон
        List<Product> productList = new ArrayList<>();
        for (Cart cart: cartList) {
            productList.add(productService.getProductId(cart.getProductId()));

        }
//        получаем цены продуктов в корзине
        float price = 0;
...
//    @PathVariable - считывает значение из url переменных
    @GetMapping("/info/{id}")
    ...
    public String deletProductFromCart(Model model, @PathVariable("id") int id){
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
//
//        // Преобразовываем объект аутентификации в специальный объект класса по работе с пользователями
        PersonDetails personDetails = (PersonDetails) authentication.getPrincipal();
                ...
                if (price.equals("sorted_by_ascending_price")) {
                    ...
                    if (!category.isEmpty()) {
                        ...
                        if (category.equals("tv")) {
                            ...
                            // Если категория равная soundbar
                            ...
                            // Если категория равная wallmount
...
//    чтобы отображалось то что до этого забили после перезагрузки страницы
...
//    @RequestParam извлекает переменные из пост
//    required = false - не факт что получим какое либо значение в выборе сортировке по цене(возвр или убыв)
